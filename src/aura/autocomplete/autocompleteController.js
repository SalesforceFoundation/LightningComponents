({
    doInit: function (component, event, helper) {
        ...
        contextOptions.forEach(function (contextOption) {
            if (contextOption.isValid()) {
                contextOption.addHandler('menuSelect', component, 'c.handleMenuSelect');
            }
        });

        helper.updateCurrentContext(component);
    },
    ...
    toggleContextMenu: function (component, event, helper) {
        helper.toggleContextMenu(component);
    },

    handleMenuSelect: function (component, event, helper) {
        helper.toggleContextMenu(component);

        var selectedContextOption = event.getParam('selectedItem');
        var selectedContext = selectedContextOption.get('v.context');

        component.get('v.contextOptions').forEach(function (contextItem) {
            var contextItemContext = contextItem.get('v.context');
            contextItem.set('v.selected', contextItemContext == selectedContext);
        });
